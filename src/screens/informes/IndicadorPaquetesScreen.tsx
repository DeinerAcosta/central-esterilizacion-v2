...
const IndicadorPaquetesScreen: React.FC = () => {
  // --- ESTADOS ---
  ...
  const [previewType, setPreviewType] = useState<any>(''); // 'Paquete' o 'Instrumental'
  
  const menuRef = useRef(null);

  const data = [
    { fecha: '20/08/2024', nombre: 'Felipe Cantillo Lara', interv: 'Plástica', qui: '8', equipo: 'Plástico', kit: '1', cant: '1' },
    { fecha: '20/08/2024', nombre: 'Pedro Padilla Gonzáles', interv: 'Plástica', qui: '8', equipo: 'Plástico', kit: '1', cant: '1' },
    { fecha: '20/08/2024', nombre: 'Ana Maria Llanos', interv: 'Plástica', qui: '8', equipo: 'Plástico', kit: '1', cant: '1' },
    { fecha: '20/08/2024', nombre: 'Camila Sotomayor', interv: 'Plástica', qui: '8', equipo: 'Plástico', kit: '1', cant: '1' },
  ];
  ...
  // 2. Descargar Fila Individual
  const handleRowDownload = (item) => {
      generateCSV([item], `reporte_${item.nombre.replace(/\s+/g, '_')}_${item.fecha.replaceAll('/', '-')}.csv`);
      setOpenMenuIndex(null);
  };

  // 3. Descarga Global
  const handleDownloadGlobal: React.FC = () => {
      generateCSV(data, "reporte_paquetes_completo.csv");
  };

  // 4. Vista Previa (Maneja dos tipos de documento)
  const handlePreview = (item, type) => {
      setSelectedItem(item);
      setPreviewType(type); // 'Paquete' o 'Instrumental'
      setIsPreviewModalOpen(true);
      setOpenMenuIndex(null);
  };

  // Funciones Modal Vista Previa
  const handlePrintDoc: React.FC = () => window.print();
  
  const handleDownloadDoc: React.FC = () => {
      const link = document.createElement("a");
      link.href = "#"; 
      link.setAttribute("download", `Documento_${previewType}.pdf`);
  ...
  const generateCSV = (rowsData, filename) => {
    const headers = "Fecha,Nombre,Intervencion,Quirofano,Equipo,Kit,Cantidad,Ind. Paquete,Ind. Instrumental\n";
    const rows = rowsData.map(row => 
      `${row.fecha},${row.nombre},${row.interv},${row.qui},${row.equipo},${row.kit},${row.cant},Documento.pdf,Documento.pdf`
    ).join("\n");
  ...
  return (
    <div className="space-y-6 h-full flex flex-col font-sans relative" onClick={() => setOpenMenuIndex(null)}>
      ...
      <h1 className="text-xl font-bold text-slate-600 pl-8 -mt-4">Indicador de paquetes e instrumentales</h1>
      ...
      <div className="bg-white rounded-[1rem] shadow-sm border border-slate-100 overflow-visible flex-1 flex flex-col">
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-blue-50/50 text-slate-700 font-bold">
              <tr>
                ...
                <th className="px-6 py-4">Quirofano</th>
                <th className="px-6 py-4">Equipo</th>
                <th className="px-6 py-4">Kit</th>
                <th className="px-6 py-4">Cantidad</th>
                <th className="px-6 py-4">Indicador del paquete</th>
                <th className="px-6 py-4">Indicador del instrumental</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {data.map((row, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors text-slate-500">
                  ...
                  <td className="px-6 py-4">{row.interv}</td>
                  <td className="px-6 py-4 text-center">{row.qui}</td>
                  <td className="px-6 py-4">{row.equipo}</td>
                  ...
                  <td className="px-6 py-4">
                      <div 
                        onClick={() => handlePreview(row, 'Paquete')}
                        className="flex items-center gap-2 text-blue-500 hover:underline cursor-pointer"
                      >
                        Documento.pdf <Eye size={16}/>
                      </div>
                  </td>

                  {/* DOCUMENTO 2: INSTRUMENTAL */}
                  <td className="px-6 py-4">
                      <div 
                        onClick={() => handlePreview(row, 'Instrumental')}
                        className="flex items-center gap-2 text-blue-500 hover:underline cursor-pointer"
                      >
                        Documento.pdf <Eye size={16}/>
                      </div>
                  ...
                  {/* --- MENÚ DE ACCIONES (Solo Editar y Descargar) --- */}
        ...
        <div className="mt-auto p-4 flex justify-between items-center text-xs text-slate-300">
            ...
            <div className="flex items-center gap-2 font-medium text-blue-400">
               ...
               <span className="bg-blue-50 px-2 py-0.5 rounded text-blue-600">1</span>/3
      ...
      {/* ======================================================= */}
      {/* MODAL 1: EDITAR (Diseño Ajustado)                       */}
      {/* ======================================================= */}
      {isEditModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 py-6 font-sans">
             ...
             {/* Ancho ajustado para acomodar 3 columnas cómodamente */}
             <div className="relative bg-white w-[900px] rounded-[30px] shadow-2xl animate-in fade-in zoom-in-95 duration-200 flex flex-col p-10">
                ...
                <div className="flex flex-col gap-6 mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <ModalInput label="Intervención" defaultValue={selectedItem?.interv} />
                    </div>

                    {/* Fila 2: Quirófano, Equipo, Cantidad */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <ModalInput label="Quirófano" defaultValue={selectedItem?.qui} />
                        <ModalInput label="Equipo" defaultValue={selectedItem?.equipo} />
                        <ModalInput label="Cantidad" defaultValue={selectedItem?.cant} />
                    </div>

                    {/* Fila 3: Indicadores (2 columnas) */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <ModalInput label="Ind. Paquete" defaultValue="Documento.pdf" icon={Paperclip} />
                        <ModalInput label="Ind. Instrumental" defaultValue="Documento.pdf" icon={Paperclip} />
                    </div>
                </div>
             ...
             <div className="relative bg-white w-[1000px] h-[650px] flex rounded-[24px] overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="w-[70%] bg-slate-100 relative flex items-center justify-center p-8">
                    <div className="bg-white shadow-xl max-w-full max-h-full overflow-hidden transform transition-transform">
                        <div className="w-[400px] h-[500px] bg-slate-200 flex items-center justify-center relative">
                             <div className="absolute inset-0 bg-[url('https://placehold.co/400x500/e2e8f0/94a3b8?text=Documento+PDF')] bg-cover opacity-50"></div>
                             <div className="bg-white p-4 shadow-md rotate-[-5deg] w-64 border border-slate-300">
                                 <div className="h-4 w-full bg-blue-500/80 mb-2"></div>
                                 <div className="flex gap-2">
                ...
                <div className="w-[30%] bg-white p-8 flex flex-col border-l border-slate-100 relative">
                    ...
                    <div className="space-y-6 flex-1">
                        <div>
                            <span className="text-blue-500 font-bold text-sm block mb-1">Tipo de Documento:</span>
                            <span className="text-slate-700 font-bold text-lg">
                                {previewType === 'Paquete' ? 'Indicador de Paquete' : 'Indicador de Instrumental'}
                            </span>
                        </div>
                        <div>
                            <span className="text-blue-500 font-bold text-sm block mb-1">Nombre Archivo:</span>
                            <div className="flex items-center gap-1 text-slate-500 text-sm bg-slate-50 p-2 rounded-lg border border-slate-100">
                                <span className="flex-1 truncate">Documento.pdf</span>
                                <span className="text-slate-400 text-xs">.PDF</span>
                            </div>
                        </div>
                        <div>
                             <span className="text-blue-500 font-bold text-sm block mb-1">Tamaño:</span>
                             <span className="text-slate-500 text-sm">245 KB</span>
                        </div>
...
export default IndicadorPaquetesScreen;