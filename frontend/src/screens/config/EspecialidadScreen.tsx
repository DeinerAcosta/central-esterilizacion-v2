...
const EspecialidadScreen: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'success',
        title: '¡Éxito!',
        text: modalMode === 'create' ? 'Especialidad creada correctamente.' : 'Especialidad actualizada correctamente.',
  ...
  const handleToggleStatus = (item: any) => {
    ...
    Swal.fire({
      title: '¿Está seguro?',
      html: `Se <b>${accion}</b> la especialidad:<br/><span style="color:#475569;font-weight:600">${item.nombre}</span>`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3b82f6',
      cancelButtonColor: '#94a3b8',
      ...
      if (result.isConfirmed) {
        try {
          ...
          await fetch(`http://localhost:4000/api/especialidades/${item.id}/estado`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
          ...
          Swal.fire({
            ...
            text: `La especialidad ha sido ${nuevoEstado ? 'habilitada' : 'deshabilitada'}.`,
            timer: 1500,
            showConfirmButton: false,
          });
          fetchEspecialidades();
          ...
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'No se pudo cambiar el estado de la especialidad.',
            confirmButtonColor: '#ef4444',
          });
        }
      }
                    ...
                    <td className="px-4 py-4 text-right relative">
                      ...
                      {openMenuIndex === i && (
                        <div ref={menuRef} className="absolute right-8 top-8 w-44 bg-white rounded-lg shadow-xl border z-50 overflow-hidden">
                          ...
                          <button onClick={() => handleToggleStatus(row)} className={`flex items-center gap-2 w-full px-4 py-2.5 text-xs hover:bg-slate-50 font-bold text-left ${row.status === 'Habilitado' ? 'text-red-500' : 'text-emerald-500'}`}>
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 44, marginTop: 8 }}>
                <button className="e-cancel-btn" onClick={handleCloseModal}>
