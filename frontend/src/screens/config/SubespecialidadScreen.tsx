...
const SubespecialidadScreen: React.FC = () => {
  ...
  const fetchListas = async () => {
    try {
      const res  = await fetch('http://localhost:4000/api/subespecialidades/listas');
  ...
  const fetchSubespecialidades = async () => {
    setLoading(true);
    try {
      const res  = await fetch(`http://localhost:4000/api/subespecialidades?page=${currentPage}&search=${searchTerm}&estado=${statusFilter}`);
      const json = await res.json();
      if (json && json.data) {
        setData(json.data.map((item: any) => ({
          ...item,
          especialidadNombre: item.especialidad?.nombre || 'N/A',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url    = modalMode === 'create'
        ? 'http://localhost:4000/api/subespecialidades'
        : `http://localhost:4000/api/subespecialidades/${formData.id}`;
  ...
  return (
    <>
      <style>{`
        ...
        .s-cancel-btn:hover { color: #ef4444; }
        ...
        <div className="flex flex-col md:flex-row gap-3 justify-between items-center bg-white p-3 rounded-xl shadow-sm border border-slate-100">
          <div className="flex gap-3 flex-1 w-full md:w-auto items-center">
            ...
            <div style={{ position: 'relative', flex: 1, maxWidth: 560 }}>
              ...
              />
              <Search className="s-search-icon" size={17} />
            </div>
          </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  ...
                  <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-500">{row.especialidadNombre}</td>
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
            ...
            <div style={{
              ...
              <h2 style={{ textAlign: 'center', fontSize: 21, fontWeight: 700, color: '#3b82f6', margin: '0 0 4px' }}>
                {modalMode === 'create' && 'Crear subespecialidad'}
                {modalMode === 'edit'   && 'Editar subespecialidad'}
              ...
              <SNotchSelect
                ...
                onChange={(v) => setFormData((p: any) => ({ ...p, especialidadId: v }))}
...
const SNotchInput: React.FC<SNotchInputProps> = ({
...
const SNotchSelect: React.FC<SNotchSelectProps> = ({
