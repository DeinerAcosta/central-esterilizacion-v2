...
const QuirofanosScreen: React.FC = () => {
  ...
  const fetchListas = async () => {
    try {
      const res  = await fetch('http://localhost:4000/api/quirofanos/listas');
  ...
  const fetchQuirofanos = async () => {
    ...
    try {
      const res  = await fetch(
        `http://localhost:4000/api/quirofanos?page=${currentPage}&search=${searchTerm}&estado=${statusFilter}`
      );
      const json = await res.json();
      if (json && json.data) {
        setData(json.data.map((q: any) => ({
          ...q,
          sedeNombre: q.sede?.nombre || 'Sin sede',
          status: q.estado ? 'Habilitado' : 'Deshabilitado'
        })));
        setTotalPages(json.totalPages || 1);
        setTotalRecords(json.total || 0);
      }
    } catch (error) { console.error('Error fetch:', error); } finally { setLoading(false); }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url    = modalMode === 'create'
        ? 'http://localhost:4000/api/quirofanos'
        : `http://localhost:4000/api/quirofanos/${formData.id}`;
  ...
  return (
    <>
      <style>{`
        ...
        .qf-save-btn:hover { opacity: 0.9; transform: translateY(-1px); }
        ...
        .qf-cancel-btn:hover { color: #ef4444; }
      ...
      <div className="space-y-6 h-full flex flex-col font-sans" onClick={() => setOpenMenuIndex(null)}>
        <h1 className="text-3xl font-bold text-blue-500">Quirófanos</h1>
        ...
        <div className="flex flex-col md:flex-row gap-3 justify-between items-center bg-white p-3 rounded-xl shadow-sm border border-slate-100">
          <div className="flex gap-3 flex-1 w-full md:w-auto items-center">
            ...
            <div style={{ position: 'relative', flex: 1, maxWidth: 560 }}>
              ...
              />
              <Search className="qf-search-icon" size={17} />
            </div>
          </div>
          ...
          >
            Crear quirófano <PlusCircle size={18} />
          </button>
        </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  ...
                  <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-500">{row.sedeNombre}</td>
                    ...
                    <td className="px-4 py-4 text-right relative">
                      ...
                      {openMenuIndex === i && (
                        <div ref={menuRef} className="absolute right-8 top-8 w-44 bg-white rounded-lg shadow-xl border z-50 overflow-hidden">
                          <button
                            onClick={() => { setFormData({ ...row, sedeId: row.sede?.id || row.sedeId }); setModalMode('edit'); setIsModalOpen(true); setOpenMenuIndex(null); }}
                            className="flex items-center gap-2 w-full px-4 py-2.5 text-xs hover:bg-slate-50 text-slate-600 font-bold text-left"
                          >
                            <Edit size={14}/> Editar
                          </button>
                          <button
                            onClick={() => { setFormData({ ...row, sedeId: row.sede?.id || row.sedeId }); setModalMode('view'); setIsModalOpen(true); setOpenMenuIndex(null); }}
                            className="flex items-center gap-2 w-full px-4 py-2.5 text-xs hover:bg-slate-50 text-slate-600 font-bold text-left"
                          >
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
            ...
            <div style={{
              ...
              <h2 style={{ textAlign: 'center', fontSize: 21, fontWeight: 700, color: '#3b82f6', margin: '0 0 4px' }}>
                {modalMode === 'create' && 'Crear quirófano'}
                {modalMode === 'edit'   && 'Editar quirófano'}
              ...
              <QFNotchSelect
                ...
                disabled={modalMode === 'view'}
                onChange={(v) => setFormData((p: any) => ({ ...p, sedeId: v }))}
...
const QFNotchInput: React.FC<QFNotchInputProps> = ({
...
const QFNotchSelect: React.FC<QFNotchSelectProps> = ({
...
export default QuirofanosScreen;