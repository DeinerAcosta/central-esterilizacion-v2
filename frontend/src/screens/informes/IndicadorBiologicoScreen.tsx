...
import { 
  ...
  Plus,    // Nuevo para zoom
  Minus    // Nuevo para zoom
} from 'lucide-react';

const IndicadorBiologicoScreen: React.FC = () => {
  // --- ESTADOS ---
  ...
  const [isPreviewModalOpen, setIsPreviewModalOpen] = useState<any>(false); // NUEVO: Estado para Vista Previa
  ...
  // Datos principales
  const data = [
    { fecha: '20/07/2025', nombre: 'Statim', valor: '5.000 + 2', kit: 'CaBas01' },
    { fecha: '05/10/2025', nombre: 'Statim', valor: '6.000 + 1', kit: 'CaBas01' },
    { fecha: '20/07/2025', nombre: 'Statim', valor: '3.000 + 4', kit: 'CaBas01' },
    { fecha: '05/10/2025', nombre: 'Statim', valor: '6.000 + 2', kit: 'CaBas01' },
  ];
  ...
  const handleRowDownload = (item) => {
      generateCSV([item], `indicador_${item.fecha.replaceAll('/', '-')}.csv`);
      setOpenMenuIndex(null);
  };

  const handleDownloadGlobal: React.FC = () => {
      generateCSV(data, "reporte_completo.csv");
  };

  // NUEVO: Funciones para el Modal de Vista Previa
  const handlePrintDoc: React.FC = () => {
      window.print(); // Abre el diálogo de impresión del navegador
  };

  const handleDownloadDoc: React.FC = () => {
      // Simula descarga de un PDF ficticio
      const link = document.createElement("a");
      link.href = "#"; // Aquí iría la URL real del documento
      link.setAttribute("download", "Documento_Evidencia.pdf");
  ...
  return (
    <div className="space-y-6 h-full flex flex-col font-sans relative" onClick={() => setOpenMenuIndex(null)}>
      ...
      <h1 className="text-xl font-bold text-slate-600 pl-8 -mt-4">Indicador biológico statim vapor</h1>
      ...
      <div className="flex flex-col xl:flex-row gap-4 justify-between items-center bg-white p-1 rounded-xl">
        ...
        <div className="flex items-center gap-4">
            <button onClick={(e: any) => { e.stopPropagation(); handleDownloadGlobal(); }} className="flex items-center gap-1 text-blue-500 font-medium text-xs hover:text-blue-600 transition-colors px-4">
            ...
            <tbody className="divide-y divide-slate-50">
              {data.map((row, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors text-slate-500">
                  ...
                  {/* Celdas que abren la Vista Previa */}
                  ...
                  <td className="px-4 py-4 text-right relative">
                    <button onClick={(e: any) => toggleMenu(i, e)} className={`p-1 rounded-full transition-colors ${openMenuIndex === i ? 'bg-blue-50 text-blue-500' : 'text-slate-300 hover:text-blue-500 hover:bg-slate-50'}`}>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 py-6 font-sans">
             ...
             <div className="relative bg-white w-[750px] rounded-[30px] shadow-2xl animate-in fade-in zoom-in-95 duration-200 flex flex-col p-10">
                <h2 className="text-2xl font-bold text-blue-500 text-center mb-8">Editar indicador biológico statim gas</h2>
      ...
      {isDetailModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 py-6 font-sans">
             ...
             <div className="relative bg-white w-[900px] rounded-[30px] shadow-2xl animate-in fade-in zoom-in-95 duration-200 flex flex-col p-8 max-h-[90vh] overflow-y-auto">
                <h2 className="text-2xl font-bold text-blue-500 text-center mb-8">Detalle indicador biológico statim gas</h2>
                {/* ... (Contenido del modal detalle igual al código anterior) ... */}
                {/* Por brevedad, mantengo la estructura del detalle que ya aprobaste */}
      ...
      {/* ======================================================= */}
      {/* MODAL 3: VISTA PREVIA (NUEVO DISEÑO SOLICITADO)         */}
      {/* ======================================================= */}
      {isPreviewModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center px-4 py-6 font-sans">
             ...
             <div className="relative bg-white w-[1000px] h-[650px] flex rounded-[24px] overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="w-[70%] bg-slate-100 relative flex items-center justify-center p-8">
                    ...
                    {/* CONTROL DE ZOOM (Inferior Izquierda) */}
                ...
                {/* 2. SECCIÓN DERECHA: INFORMACIÓN Y ACCIONES */}
                <div className="w-[30%] bg-white p-8 flex flex-col border-l border-slate-100 relative">
                    ...
                    <div className="space-y-6 flex-1">
                        ...
                        <div>
                            ...
                            <span className="text-slate-500 text-sm">Contabilidad/Causación</span>
                        </div>
                        ...
                        <div>
                             ...
                             <span className="text-slate-500 text-sm">50 KB</span>
                        </div>

                        {/* Acciones: Imprimir y Descargar */}
...
export default IndicadorBiologicoScreen;